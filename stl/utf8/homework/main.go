...
func main() {
	fmt.Println("Количество символов:", len(data))

	r := strings.NewReplacer("🙂", "=)", "👨‍💻", "=)", "🏢", "=)", "💵", "=)", "🌏", "=)", "💼", "=)", "✅ ", "=)", "📌", "=)", "🔥", "=)", "✍️", "=)", "📫", "=)")
	noSmiles := r.Replace(data)

	fmt.Println("Количество символов после замены смайлов:", len(noSmiles))

	translit := transliterate(data)
	fmt.Println("Количество символов после транслитерации:", len(translit))

	cyrillicBytes := utf8.RuneCountInString(data)
	latinBytes := utf8.RuneCountInString(translit)
	compression := float64(latinBytes) / float64(cyrillicBytes)
	fmt.Println("Компрессия текста:", compression)
}
